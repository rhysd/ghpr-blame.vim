...
function! s:_git(...) dict abort
    ...
    let out = system(cmd)
    if v:shell_error
        call s:throw(printf("Git command '%s' failed: %s", cmd, out))
    endif
    return out
endfunction
...
function! s:_start() dict abort
    if &l:buftype =~# 'nofile\|help\|quickfix' || self.file ==# ''
        call s:error('Invalid file for running blame')
        return
    endif

    let slug = self.extract_slug()
    if slug ==# ''
        call s:warn('Cannot get GitHub repository from')
        return
    endif
    let self.slug = slug
    ...
    if mapping !=# ''
        execute 'nnoremap <buffer><silent>' . mapping . ' :<C-u>call <SID>show_pr_at_current_line()<CR>'
    endif
endfunction
...
function! s:_show_pr_at(line) dict abort
    ...
    if !self.started || idx >= (len(self.blames)-1) || !has_key(self.blames[idx], 'pr')
        call s:warn('no PR is related to this line')
        return
    endif
...
function! s:_fetch_pr(num) dict abort
    ...
    let response = s:H.request({
        ...
        \ })
    if !response.success
        call s:error(printf('API request failed with status %s: %s', response.status, response, statusText))
        return {}
    endif
...
function! s:show_pr_at_current_line() abort
